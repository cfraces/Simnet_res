...
class WaveTrain(TrainDomain):
  def __init__(self, **config):
    ...
    source = f0**2 * gaussian * (1-2*pi**2*f0**2*(t_symbol-1/f0)**2)*exp(-pi**2*f0**2*(t_symbol-1/f0)**2)
    ...
    interior_hr = geo_hr.interior_bc(outvar_sympy={'wave_equation': -source},
                                  bounds={x: (0.5-hr_radius, 0.5+hr_radius), y: (0.5-hr_radius, 0.5+hr_radius)},
...
class WaveSolver(Solver):
  ...
  def __init__(self, **config):
    ...
    self.equations = (WaveEquation(u='z', c=1.0, dim=2, time=True).make_node()
                    + [Node.from_sympy(Symbol('t')**2 * Symbol('z_star'), 'z')]) # enforce "hard" boundary conditions
  ...
  def update_defaults(cls, defaults):
    defaults.update({
        'network_dir': './network_checkpoint_wave_2d',
