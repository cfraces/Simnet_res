...
from simnet.architecture.dgm import DGMArch
...
class BuckleyVal(ValidationDomain):
  def __init__(self, **config):
    ...
    w = sio.loadmat('./buckley/Buckley_sin_Swc0_Sor_0_M_2.mat')
...
class BuckleySolver(Solver):
  ...
  def update_defaults(cls, defaults):
    defaults.update({
      'network_dir': './checkpoint_batch/buckley_dgm_{}'.format(int(time.time())),
      'max_steps': 30000,
      'decay_steps': 300,
