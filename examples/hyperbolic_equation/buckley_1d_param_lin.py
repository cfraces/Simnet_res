...
# v1
ref_buckley_v_1 = sio.loadmat('./buckley/Buckley_affine_vel_1.mat')
...
# v50
ref_buckley_v_50 = sio.loadmat('./buckley/Buckley_affine_vel_0.5.mat')
...
# v2
ref_buckley_v_2 = sio.loadmat('./buckley/Buckley_affine_vel_2.mat')
...
# v10
ref_buckley_v_10 = sio.loadmat('./buckley/Buckley_affine_vel_10.mat')
u10 = np.expand_dims(ref_buckley_v_10['usol'].flatten(), axis=-1)
outvar_v_10_numpy = {'u': u10}
ud_10 = np.ones_like(X) * 10.0 + 1
...
class BuckleyInference(InferenceDomain):
  def __init__(self, **config):
    ...
    # save entire domain
    # Normal distribution
    sample_vel = get_truncated_normal(mean=4, sd=1, low=0.1, upp=10).rvs(10)
...
class BuckleySolver(Solver):
  ...
  def update_defaults(cls, defaults):
    defaults.update({
      'network_dir': './checkpoint_affine/buckley_param_wide_init_{}'.format(int(time.time())),
      'max_steps': 90000,
      'decay_steps': 300,
      'start_lr': 3e-4,
      'amp': True,
    ...
    # defaults.update({
    #   'network_dir': './checkpoint_affine/buckley_param_wide_{}'.format(int(time.time())),
    #   'initialize_network_dir': './checkpoint_affine/buckley_param_wide1632408286',
